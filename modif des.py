...
# feistel returns f_out and details dictionary for logging
def feistel(R, K):
    details = {}
    E_R = permute(R, E)
...
def generate_subkeys(key_bytes):
    ...
    key_bits = str_to_bits(key_bytes)
    pc1 = permute(key_bits, PC_1)
    C = pc1[:28]
    D = pc1[28:]
    subkeys = []
    for s in SHIFTS:
...
# updated des_encrypt_block accepts optional key_labels list to print the real label used
def des_encrypt_block(block_bytes, subkeys, key_labels=None):
    ...
    for rnd in range(16):
        ...
        # determine label for this round
        if key_labels:
...
def encrypt_gui():
    ...
    # normalize key to 8 bytes: pad with zeros or truncate
    if len(key) > 8:
        key = key[:8]
    key_bytes = key.encode('utf-8')

    subkeys, pc1 = generate_subkeys(key_bytes)
    # build labels for keys (K01..K16)
...
def decrypt_gui():
    ...
    subkeys, pc1 = generate_subkeys(key_bytes)
    # Build labels, but reversed for display in decrypt (K16..K01)
    ...
    for i in range(0, len(cipher_bytes), 8):
        ...
        # ensure block is 8 bytes (if not, it's an input error)
        if len(block) < 8:
        ...
        for lg in logs:
            output.insert(tk.END, lg + "\n")
        output.insert(tk.END, f"Plain block (bin): {''.join(map(str,pbits))}\n\n")
    ...
    try:
        ...
        plain_result = bytes(plain_bytes_all).decode('utf-8', errors='replace')

    output.insert(tk.END, "=== HASIL AKHIR (DECRYPT) ===\n")
    output.insert(tk.END, f"Plaintext: {plain_result}\n")
    output.configure(state='disabled')

    # place plaintext back into entry_plain for convenience
...
root = tk.Tk()
root.title("DES Encryption - GUI Version")
